...
for row in result:
    name = row[0]
    if 'MSDU' in name or 'VLAN' in name or '应用层' in name or '管理消息' in name or 'IPV4' in name:
        print(f"{name:25s} | {row[1]:20s} | {row[2]:10s} | {row[3]}")
...
for row in result:
    if row[0] == '原始源TEI':
        print(f"\n原始源TEI: {row[2]}")
    if row[0] == 'MSDU类型':
        print(f"MSDU类型: {row[2]} ({row[3]})")
    if row[0] == 'VLAN标签':
        print(f"VLAN标签: {row[2]}")