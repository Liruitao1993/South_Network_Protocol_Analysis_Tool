...
print(f"Frame bytes 32-40: {frame_bytes[32:40].hex()}")
print(f"Frame bytes 20-40: {frame_bytes[20:40].hex()}")

parser = CSGNewGenParser()
result = parser.parse_to_table(frame_bytes)

for i, row in enumerate(result):
    if 'MSDU' in row[0] and '负载' in row[0]:
        print(f"[{i}] {row[0]} | {row[1]} | {row[2]} | {row[3]}")
        
# Check what mac_data looks like
mac_data = frame_bytes[20:]
print(f"mac_data length: {len(mac_data)}")
print(f"mac_data[0]: {mac_data[0]:02X}")
print(f"mac_data[32]: {mac_data[32]:02X}")
print(f"mac_data[32:40]: {mac_data[32:40].hex()}")